...
def run_swevo_experiments(output_dir="swevo/v2"):
    ...
    for algo_name, algo_fn in algos.items():
        if algo_name == "AOBL-SOS":
            v, w, _ = algo_fn(obj, pop_base.copy(), map_func, iters=500, is_portfolio=True, init_obl=True, obl_mode="portfolio_reversal", cap=cap)
        else:

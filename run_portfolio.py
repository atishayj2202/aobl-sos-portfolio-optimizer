...
def run_jpm_experiments(output_dir="jpm/v3"):
    ...
    _, w_aobl, _ = AOBL_SOS(obj, pop.copy(), map_func, iters=500, is_portfolio=True, init_obl=True, obl_mode="quasi", lb=0.0, ub=1.0, cap=cap)
    
    print("  Running Standard SOS...")
    _, w_sos, _ = SOS(obj, pop.copy(), map_func, iters=500, is_portfolio=True)
    
    print("  Computing Min Variance...")
    w_minvar = map_func(min_variance_portfolio(cov, cap=cap))
    
    print("  Computing Max Sharpe (Markowitz)...")
    w_maxsh = map_func(max_sharpe_portfolio(mu, cov, rf_daily, cap=cap))
    
    print("  Computing Inverse Volatility (Risk Parity)...")
    w_invvol = map_func(inverse_volatility_portfolio(cov, cap=cap))
    ...
    for split_name, train_end, test_end in splits:
        ...
        _, w_wf_aobl, _ = AOBL_SOS(obj_w, pop_w, map_func, iters=200, is_portfolio=True, init_obl=True, obl_mode="quasi", lb=0.0, ub=1.0, cap=cap)
